<template>
  <div>
    <el-row>
      <el-col>
        <el-button @click="showCreateDialog" style="float: right" type="primary" size="mini">添加portfolio</el-button>
      </el-col>
    </el-row>
    <el-table :data="list">
    ...
    <el-dialog class="dialog" :visible.sync="dialogVisible" title="添加Portfolio">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPortfolioList() {
        request.getPortfolioList().then(data => {
          this.list = data.list
        })
      },
      onSubmit() {
      ...
      createPortfolio() {
        request.createPortfolio(this.form).then(data => {
      ...
      updatePortfolio() {
        request.updatePortfolio(this.form).then(data => {
          this.dialogVisible = false
          this.$message({
            type: 'success',
            message: '更新成功'
      ...
      deletePortfolio(id) {
        request.deletePortfolio({ id }).then(data => {
          this.$message({
            type: 'success',
            message: '删除成功'
          })
      ...
      onDelete(index, row) {
        this.$confirm('确定删除该Portfolio?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.deletePortfolio(row.id)
        })
      }
    }
  }
